...
dif_data = {x: pos_data[x] - neg_data[x] for x in pos_data if x in neg_data}
# choose a color for each state based on sentiment.
pos_colors = {}
statenames = []
pos_cmap = plt.cm.Greens # use 'hot' colormap

vmin = min(pos_data.values()); vmax = max(pos_data.values());# set range.
...
vmin = min(neg_data.values()); vmax = max(neg_data.values());# set range.
for shapedict in m.states_info:
    statename = shapedict['NAME']
    # skip DC and Puerto Rico.
    if statename not in ['District of Columbia', 'Puerto Rico']:
...
for nshape, seg in enumerate(m.states):
    ...
    if statenames[nshape] not in ['District of Columbia', 'Puerto Rico']:
        ...
        poly = Polygon(seg, facecolor=color, edgecolor=color)
        ax.add_patch(poly)
plt.title('Negitive Trump Sentiment Across the US')
plt.savefig("negmap.png")

# choose a color for each state based on sentiment.
dif_colors = {}
statenames =[]
dif_cmap = plt.cm.Blues # use 'hot' colormap

vmin = min(dif_data.values()); vmax = max(dif_data.values());# set range.
for shapedict in m.states_info:
    statename = shapedict['NAME']
    # skip DC and Puerto Rico.
    if statename not in ['District of Columbia', 'Puerto Rico']:
        dif = dif_data[statename]
        dif_colors[statename] = dif_cmap(1. - np.sqrt(( dif - vmin )/( vmax - vmin)))[:3]
    statenames.append(statename)
# cycle through state names, color each one.
...
for nshape, seg in enumerate(m.states):
    # skip Puerto Rico and DC
    if statenames[nshape] not in ['District of Columbia', 'Puerto Rico']:
        color = rgb2hex(dif_colors[statenames[nshape]]) 
        poly = Polygon(seg, facecolor=color, edgecolor=color)
        ax.add_patch(poly)
plt.title('Difference in Trump Sentiment Across the US')
plt.savefig("difmap.png")
...
ax1.scatter(story['submission_score'], story['pos_percentage'], s=10, c='b', marker="s", label='Positive')
ax1.scatter(story['submission_score'], story['neg_percentage'], s=10, c='r', marker="o", label='Negative')
...
ax1.scatter(story['comment_score'], story['pos_percentage'], s=10, c='b', marker="s", label='Positive')
ax1.scatter(story['comment_score'], story['neg_percentage'], s=10, c='r', marker="o", label='Negative')
plt.legend(loc='lower right');
